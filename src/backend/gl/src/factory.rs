...
use {gl, tex};
use core::{self as d, buffer, factory as f, mapping, texture as t};
...
use {OutputMerger, Resources as R, Share};
use {Buffer, BufferElement, FatSampler, Fence, NewTexture, PipelineState, ResourceView, TargetView};
...
impl Factory {
    ...
    fn init_buffer(
        ...
        mapping_access.map(|access| {
            ...
            if let Err(err) = self.share.check() {
                panic!(
                    "Error {:?} mapping buffer: {:?}, with access: {:?}",
                    err, info, access
                )
            }
...
pub fn temporary_ensure_mapped(
...
pub fn temporary_ensure_unmapped(
    pointer: &mut *mut ::std::os::raw::c_void,
    target: gl::types::GLenum,
    buffer: Buffer,
    gl: &gl::Gl,
...
impl f::Factory<R> for Factory {
    ...
    fn create_texture_raw(
        ...
        let gl = &self.share.context;
        let object = if desc.bind
            .intersects(Bind::SHADER_RESOURCE | Bind::UNORDERED_ACCESS)
            || data_opt.is_some()
        {
            let name = if caps.immutable_storage_supported && desc.usage == Usage::Dynamic {
    ...
    fn view_buffer_as_shader_resource_raw(
        ...
        let format = tex::format_to_glfull(format).map_err(|_| f::ResourceViewError::Unsupported)?;
        unsafe {
        ...
        T: Copy,
    {
        ...
        unsafe {
            mapping::read(buf.raw(), |mapping| match mapping.kind {
                ...
                MappingKind::Temporary => temporary_ensure_mapped(
                    &mut mapping.pointer,
